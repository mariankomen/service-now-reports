...
interface CreateReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (name: string, folderId: string, description: string) => void;
  folders: Folder[];
}

const CreateReportModal: React.FC<CreateReportModalProps> = ({ isOpen, onClose, onCreate, folders }) => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    onCreate(name, folderId, description);
    setName('');
    onClose();
  };

  const handleNameChange = (e: ChangeEvent<HTMLInputElement>) => setName(e.target.value);
  const handleParentChange = (e: ChangeEvent<HTMLSelectElement>) => setFolderId(e.target.value);

  return (
    <div style={overlayStyle}>
      <div style={modalStyle}>
        <h3 style={{ margin: '0 0 15px 0' }}>New Report</h3>
        <form onSubmit={handleSubmit}>
          ...
          <div style={fieldStyle}>
            <label style={labelStyle}>Save in Folder:</label>
            <select
              value={folderId}
...
const modalStyle: React.CSSProperties = { fontFamily: 'sans-serif', backgroundColor: 'white', padding: '24px', borderRadius: '3px', width: '400px', boxShadow: '0 8px 16px -4px rgba(9, 30, 66, 0.25), 0 0 1px rgba(9, 30, 66, 0.31)' };
